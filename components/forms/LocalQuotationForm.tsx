...
import { useLocalQuotationImage } from "@/hooks/use-form-hook";
...
export type LocalQuotationFormSchema = z.infer<typeof formSchema>;

const LocalQuotationForm = () => {
  const { mutate } = useLocalQuotationImage();
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    mutate(values, {
      onSuccess: (blob) => {
        ...
        a.href = url;
        a.download = "local-quotation.png";
  ...
  useEffect(() => {
    if (totalApproxCost) {
      form.setValue("advance", totalApproxCost / 2);
    }
            ...
            render={({ field }) => (
              <FormItem className="grid grid-cols-[1fr_3fr] gap-4">
                <FormLabel className="mt-2 flex items-center justify-end text-center">
                  Pay 50% in Advance to confirm trip (₹)
                </FormLabel>
                <FormControl>
                  <Input
                    className="shad-input bg-white"
...
export default LocalQuotationForm;
